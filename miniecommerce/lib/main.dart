...
// ------------------- MODELOS DE DADOS -------------------
// Usamos classes para organizar os dados do nosso app.
class Produto {
   final String nome;
   final String descricao;
   final List<String> caracteristicas;
   final double preco;
   bool isSelecionado; // Usado para o Checkbox
   Produto({
      required this.nome,
          ...
          child: Card(
              ...
              child: Padding(
                ...
                child: Column(
                    ...
                    children: [
                      const Text('Bem-vindo à Tech Shop', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
...
class _TelaListaProdutosState extends State<TelaListaProdutos> {
   // Lista de produtos (estado da tela)
   final List<Produto> _produtos = [
      Produto(nome: 'Notebook Ultra Power', descricao: 'Ideal para tarefas pesadas.', caracteristicas: ['16GB RAM', 'SSD 512GB', 'Tela 4K'], preco: 7500.00),
      Produto(nome: 'Smartphone NextGen', descricao: 'Fotografia profissional no seu bolso.', caracteristicas: ['Câmera 108MP', 'Tela AMOLED', '5G'], preco: 4500.00),
      Produto(nome: 'Fone WaveSound', descricao: 'Imersão total com cancelamento de ruído.', caracteristicas: ['Bluetooth 5.2', '20h de bateria', 'Case de recarga'], preco: 800.00),
   ];

   @override
   Widget build(BuildContext context) {
      // Recebe o login da tela anterior
      ...
      floatingActionButton: FloatingActionButton.extended(
         ...
         onPressed: () {
            // Filtra a lista para pegar apenas os produtos selecionados
            final produtosSelecionados = _produtos.where((p) => p.isSelecionado).toList();
                Navigator.pushNamed(
                  context,
                  '/carrinho',
         ...
         body: Card(
            ...
            child: Padding(
               ...
               child: Column(
                  ...
                  mainAxisSize: MainAxisSize.min,
                  children: [
                     Text(produto.descricao, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                     const SizedBox(height: 10),
                     ...produto.caracteristicas.map((c) => Text('• $c')).toList(),
                     const SizedBox(height: 20),
                     Text('R\$ ${produto.preco.toStringAsFixed(2)}', style: const TextStyle(fontSize: 24, color: Colors.indigo)),
                  ],
               ),
             ),
        ),
         ...
         body: Padding(
            ...
            child: Column(
               children: [
                 ...
                 const Divider(),
                 TextField(controller: _enderecoController, decoration: const InputDecoration(labelText: 'Endereço de Entrega', border: OutlineInputBorder())),
                 const SizedBox(height: 20),
                 ElevatedButton(
                    ...
                    onPressed: () {
                       Navigator.pushNamed(
                          context,
                          '/finalizar',
                          arguments: {'login': login, 'produtos': produtos, 'total': total, 'endereco': _enderecoController.text},
                       );
                    },
                 ),
              ],
...
class TelaFinalizarCompra extends StatelessWidget {
   ...
   @override
   Widget build(BuildContext context) {
      final Map<String, dynamic> dadosFinais = ModalRoute.of(context)!.settings.arguments as Map<String, dynamic>;
      return Scaffold(
         ...
         body: Card(
            ...
            child: Padding(
               ...
               crossAxisAlignment: CrossAxisAlignment.start,
               children: [
                  Text('Obrigado, ${dadosFinais['login']}!', style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
                  const Divider(height: 30),
                  const Text('Resumo do Pedido:', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                  // Exibe os produtos e o total (já pronto)
                  ... (dadosFinais['produtos'] as List<Produto>).map((p) => ListTile(title: Text(p.nome), trailing:
                  Text('R\$ ${p.preco.toStringAsFixed(2)}'))).toList(),
                  Text('Total: R\$ ${(dadosFinais['total'] as double).toStringAsFixed(2)}', style: const
